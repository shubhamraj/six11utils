...
/**
 * A programmable Finite State Machine implementation. To use this
 * class, establish any number of states with the 'addState'
 * method. Next, add some FSM.Transition objects (the Transition class
 * is designed to be used as an superclass for your anonymous
 * implementation). Each Transition object has two useful methods that
 * can be defined by your implementation: doBeforeTransition and
 * doAfterTransition. To drive your FSM, simply give it events using
 * the addEvent method with the name of an event. If there is an
 * appropriate transition for the current state, the transition's
 * doBefore/doAfter methods are called and the FSM is put into the new
 * state. It is legal (and highly useful) for the start/end states of
 * a transition to be the same state.
 **/
public class FSM { // This class implements a Flying Spaghetti Monster
  ...
  /**
   * Establish a new state the FSM is aware of. If the FSM does not
   * currently have any states, this state becomes the current,
   * initial state. This is the only way to put the FSM into an
   * initial state.
   *
   * The entryCode, exitCode, and alwaysRunCode are Runnables that the FSM
   * executes during the course of a transition. entryCode and
   * exitCode are run only if the transition is between two distinct
   * states (i.e. A->B where A != B). alwaysRunCode is executed even if
   * the transition is re-entrant (i.e. A->B where A = B).
   **/
  public void addState(String state, Runnable entryCode, Runnable exitCode, 
    ...
    if (!states.containsKey(state)) {
      states.put(state, new State(state, entryCode, exitCode, alwaysRunCode));
    }
    if (isInitial) {
      setState(state);
    }
  ...
  /**
   * There are cases where a state is meant to be transitional, and
   * the FSM should always immediately transition to some other
   * state. In those cases, use this method to specify the start and
   * end states. After the startState has fully transitioned (and any
   * change events have been fired) the FSM will check to see if there
   * is another state that the FSM should automatically transition
   * to. If there is one, addEvent(endState) is called.
   *
   * Note: this creates a special transition in the lookup table
   * called "(auto)".
   */
  ...
  /**
   * Sets the current state without following a transition. This will
  ...
  /**
   * Sets the current state without followign a transition, and
   * optionally causing a change event to be triggered. During state
   * transitions (with the 'addEvent' method), this method is used
   * with the triggerEvent parameter as false.
   *
  ...
  /**
   * Establish a new transition. You might use this method something
   * like this:
   *
   * fsm.addTransition(new FSM.Transition("someEvent", "firstState", "secondState") {
   *   public void doBeforeTransition() { System.out.println("about to transition..."); }
   *   public void doAfterTransition() { fancyOperation(); }
   * });
   */
  ...
  /**
   * Add a change listener -- this is a standard java change listener
   * and is only used to report changes that have already
   * happened. ChangeEvents are only fired AFTER a transition's
  ...
  /**
   * Feed the FSM with the named event. If the current state has a
   * transition that responds to the given event, the FSM will
   * performed the transition using the following steps, assume start
   * and end states are A and B:
   *
   * <ol>
   ...
   *  <li>Now firmly in state B, see if B has a third state C that we
   *  must automatically transition to via addEvent(C).</li>
   * </ol>
   */
  ...
  private static class State {
    ...
    State(String name, Runnable entryCode, Runnable exitCode,
	  Runnable alwaysRunCode) {
      this.name = name;
  ...
  public static class Transition {
    ...
    /**
     * Create a transition object that responds to the given event
     * when in the given startState, and puts the FSM into the
     * endState provided.
     */
    ...
    /**
     * Override this to have FSM execute code immediately before
     * following a state transition.
     */
    public void doBeforeTransition() { }

    /**
     * Override this to have FSM execute code immediately after
     * following a state transition.
     */
