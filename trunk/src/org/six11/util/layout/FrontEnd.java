...
/**
 * FrontEnd is a layout manager/container that is designed to work with an XML
 * input file that describes the relative locations of various components.
 *
 * Note that XML is not directly supported in this class (because in many
 * situations you don't want to have to package JDOM up with your app just to
 * use this layout manager). If you want to use XML, use FrontEndXML, which is a
 * subclass of this.
 *
 * To use this class, first add your components to using add(Component, String),
 * where the String is the name that will forever be used to identify the
 * component you just added. Example:
 *
 * <pre>
 * fe.add(left, &quot;Left&quot;);
 * fe.add(mid, &quot;Mid&quot;);
 ...
 * </pre>
 *
 * Next (and it is important that this happens afterwards), you must give the
 * FrontEnd a list of rules concerning how things are related. You can do this
 * in one of two ways:
 *
 ...
 * </pre>
 *
 * The first line means "Wherever Root's east anchor is, make Right's east
 * anchor be 4 pixels away." (Note that the pixel distance is in the direction
 * of the relation, so if we were going from south to north, this would mean 4
 * pixels above). The second line means "Wherever Left's east anchor is, make
 * Mid's west anchor line up with it exactly."
 *
 * The other way to assign relationship rules is to use an XML file. Use the
 * FrontEndXML class for this. Like the manual method, the names that you gave
 * your components in the add() section will be used here. To use an XML file,
 * call either
 *
 * <pre>
 * load(String)
 ...
 * <pre>
 * load(InputStream)
 * </pre>. Here is a snippet of XML that is equivalent to the addRule
 * statements above:
 *
 * <pre>
 *  &lt;frontend&gt;
 ...
 * </pre>
 *
 * The benefit to using the XML file is that you can use an external tool to
 * work out the layout (or directly edit the XML). This helps keep your GUI code
 * cleaner, and it also means you don't have to recompile anything when you want
 * to tweak the layout.
 */
public class FrontEnd extends JComponent {
     ...
     */
    public static final Anchor S = Anchor.makeAnchor("South", CARDINAL_S,
            CARDINAL_N);

    /**
     * Anchor for referring to the horizontal center of some component.
     */
    public static final Anchor H = Anchor.makeAnchor("Horizontal Center", "H",
            "H");
     ...
     */
    public static final Anchor E = Anchor.makeAnchor("East", CARDINAL_E,
            CARDINAL_W);

    /**
     * Anchor for referring to the left (west) of some component.
     */
    public static final Anchor W = Anchor.makeAnchor("West", CARDINAL_W,
            CARDINAL_E);

    /**
     * Anchor for referring to the top (north) of some component.
     */
    public static final Anchor N = Anchor.makeAnchor("North", CARDINAL_N,
            CARDINAL_S);
    ...
    protected EdgeCallback onCycle = new EdgeCallback() {
            public void run(Edge e) {
                error("Edge " + e +
                    " is dependent on itself in some way. Mode: " +
                    e.getMode());
            }
        };

    /**
     * Called when there is a collision in parsing and a second look is in
     * order. This may indicate a problem, but it may be OK too.
     */
    ...
    /**
     * Create a new FrontEnd layout with no constraints or components. See the
     * constructor with the debugID for more info.
     */
    public FrontEnd() {
    ...
    /**
     * Create a new FrontEnd with a debugging ID. The other constructor defers
     * to this.
     */
    ...
    /**
     * Like the other addRule methods, but instead of using a direct reference
     * to anchors, you provide the anchor's name.
     */
    ...
    /**
     * Constrains two named components anchors so that they are joined with no
     * space between them.
     */
    public void addRule(String parentName, Anchor parentAnchor,
        String childName, Anchor childAnchor) {
        addRule(parentName, parentAnchor, childName, childAnchor, 0);
    }

    /**
     * Constrains two named components so that they are joined with some amount
     * of offset between them. <b>Note</b> that the components to which you are
     * referring should have already been added to the layout using the
     * add(Component, String) method.
     */
    public void addRule(String parentName, Anchor parentAnchor,
        String childName, Anchor childAnchor, int offset) {
    ...
    protected void traverseEdgeForLayout(Edge e) {
        ...
        int prefDist = edgeData.getPrefDistance(1.0);
        ...
        if (boxes.containsKey(childNA.name)) {
            ((Box) boxes.get(childNA.name)).set(childNA.anchor,
                childNA.coordinate);
        } else {
            error("I don't have a box for " + childNA.name);
        }
    }
    ...
    protected boolean isWrongWay(NamedAnchor p, NamedAnchor c, EdgeData d) {
        ...
        } else if (p.name.equals(ROOT) &&
                ((p.anchor.equals(S)) || p.anchor.equals(E))) {
            ret = true;
        } else if (!p.name.equals(c.name)) {
            // explicit edges for distinct boxes that go p.N->c.S or
            // p.W->c.E are 'wrong way'
            ret = ((p.anchor.equals(N) && c.anchor.equals(S)) ||
                (p.anchor.equals(W) && c.anchor.equals(E)));
        }
    ...
    protected void traverseEdgeForCorrectness(Edge e) {
        ...
        if (computePref) {
            ...
            if (!start.equals(end)) {
                ...
                if (dist.isVertical()) {
                    end.dist = comp.isVisible()
                        ? comp.getPreferredSize().height : 0;
                } else {
                    end.dist = comp.isVisible() ? comp.getPreferredSize().width
                                                : 0;
                }
    ...
    protected PrefData makePrefData(NamedAnchor a, Graph g) {
        ...
        if (a.name.equals(ROOT)) {
            if (a.anchor.equals(N)) {
                ...
                error("Root namedanchor isn't a cardinal direction?? " + a);
            }
        } else {
            pd.name = a.name;
        }
        ...
        if (list.size() == 0) {
            ...
            error("Apparently there is more than one node for " + pd);
        }
    ...
    public static Dimension parseDimension(String wwXhh) {
        ...
        try {
            ...
            ret.height = Integer.parseInt(wwXhh.substring(wwXhh.indexOf("x") +
                        1));
        } catch (Exception ex) {
            error("Issues with parseDimension input: " + wwXhh);
        }
    ...
    public static Color parseColor(String rrggbb) {
        ...
        if (val.length() != 6) {
            error("Issues with parseColor input: " + rrggbb);
        } else {
            try {
    ...
    /**
     * Returns the graph used to hold the bulk of the component's size and
     * location data.
     */
    ...
    protected Edge makeEdge(Node a, Node b, boolean derived, int offset) {
        ...
        if (derived) {
            ret = new Edge(a, b,
                    new EdgeData((Component) map.get(parentNA.name),
                        parentNA.isVertical()));
        } else {
    ...
    class TreeCB extends EdgeCallback {
        public void run(Edge e) {
            ...
            thisOut("(tree) " + white + " now " + whitePD.runningValue +
                "    sameAnchor: " + edgePD.sameAnchor);
        }
    }
    ...
    class BackCB extends EdgeCallback {
        public void run(Edge e) {
            ...
            thisOut("(back) " + oldPD + " now " + oldPD.runningValue);
        }
    }
    ...
    class NodeCB extends NodeCallback {
        public void run(Node n) {
            ...
            thisOut("(node) " + n + " now " + d.runningValue);
        }
    }
    ...
    class PrefData {
        ...
        boolean isCardinal() {
            return name.equals(CARDINAL_N) || name.equals(CARDINAL_E) ||
            name.equals(CARDINAL_W) || name.equals(CARDINAL_S);
        }
    }
    ...
    class FrontEndLayout implements LayoutManager2 {
        ...
        /**
         * Not implemented, since removing components from a layout that is
         * based on the relationships between components would bring the world
         * crashing down. If you need to simulate removing a component, use a
         * Holder.
         */
        ...
        /**
         * Returns the preferred dimensions for this layout for all the visible
         * components contained in the parent container.
         *
         * This essentially equates the the preferred size of all the visible
         * components within the layout, plus whatever padding may be specified
         * between them, plus insets surrounding the parent.
         */
        ...
        /**
         * Used during preferred-size and for layout itself. When doing just
         * preferred-size computing, set applyBounds to false in order to avoid
         * changing the positions/sizes of components.
         */
        protected void performLayout(Container parent, boolean applyBounds) {
            ...
            if ((preferredSize == null) || (vertPrefGraph == null) ||
                    (horizPrefGraph == null)) {
            ...
            if (computePref) {
                ...
                if (debugging) {
                    thisOut("*** Preferred Size of component: " +
                        FrontEnd.dim(preferredSize));
                    thisOut("Vertical Graph:");
                    thisOut(
                        "TODO: removed printAdjacency at some point. Bit rot strikes!");
                    //                    thisOut(vertPrefGraph.printAdjacency(false));
                    thisOut("Horizontal Graph:");

                    // thisOut(horizPrefGraph.printAdjacency(false));
                }
            }
                ...
                if (applyBounds) {
                    for (Iterator it = boxes.values().iterator(); it.hasNext();) {
                        ...
                        if (!b.name.equals(ROOT) && b.isDone()) {
                            ...
                            String message = "Box for " + c.getName() +
                                " is: " + rect(b.getBounds()) + " visible? " +
                                c.isVisible();

                            if (c instanceof Holder) {
                                message += (" This is a holder. Held component visible? " +
                                ((Holder) c).isComponentVisible());
                            }
        ...
        /**
         * Returns the largest PrefData.runningValue among all of the cardinal
         * directions in the given graph. Each graph should only contain N/S
         * directions or E/W directions, but not both.
         */
        ...
        /**
         * Associates the name (must be a String to be effective) with the
         * component. When a container is layed out, this name will be used to
         * find the component.
         */
        ...
        /**
         * There is no limit to the size of a FrontEnd other than what can fit
         * into an int.
         */
        ...
        /**
         * Returns 0.5f, indicating it should be horizontally centered within
         * it's parent.
         */
        ...
        /**
         * Returns 0.5f, indicating it should be vertically centered within it's
         * parent.
         */
        ...
        /**
         * De-caches information related to layout so it is forced to compute
         * things from scratch.
         */
    ...
    protected static class EdgeData {
        ...
        public static boolean linksRoot(Edge e) {
            return (((NamedAnchor) e.a.data).isRoot() ||
            ((NamedAnchor) e.b.data).isRoot());
        }
    }
    ...
    protected static class NamedAnchor {
        ...
        NamedAnchor(String name, Anchor anchor) {
            this.name = name;
            this.anchor = anchor;
            vertical = (anchor.equals(N) || anchor.equals(V) ||
                anchor.equals(S));
        }
        ...
        /**
         * If the connection between 'this' and 'other' goes from N to S or W to
         * E this returns true. Otherwise returns false.
         */
        protected boolean isNSWE(NamedAnchor other) {
            return ((anchor.equals(N) && other.anchor.equals(S)) ||
            (anchor.equals(W) && other.anchor.equals(E)));
        }
    ...
    public static class Anchor {
        ...
        protected static Anchor makeAnchor(String name, String shortName,
            String complementShortName) {
            Anchor ret = null;
