...
/**
 * This is a whole slew of static methods that operate on points,
 * lines, vectors, and point sequences.
 **/
public abstract class Functions {
  ...
  /**
   * Given some sequence of points (or subsequence) return the average
   * vector that represents the change in position.
   */
  ...
  public static Pt getCircleCenter(Pt a, Pt b, Pt c) {
    ...
    if (G == 0.0) return null; // a, b, c must be collinear
  ...
  /**
   * Gives you the curvature at a point. It computes the midpoint of
   * the circle that is defined by the three points (if such a circle
   * exists; if the three points are collinear, there is no
   * circle). The curvature returned is 0 for a straight line. For
   * nonstraight samples, the curvature is signed. If you were to walk
   * in a straight line from point a to point c, b will either be on
   * your left (negative curvature) or on your right.
   */
...
//   }
  ...
  /**
   * Returns the intersection point of the two given lines. If they
   * are colinear or parallel this will return null.
   */
  ...
  /**
   * Returns the point (or the oldest point, in the event of a tie) on
   * the given sequence that is nearest the provided epicenter point.
   */
  ...
  public static double getMinDistBetweenPointsOnSequence(Sequence seq, Pt a, Pt b) {
    ...
    Pt prev = null;
    for (Pt pt : seq) {
      if ((prev != null) && (foundA ^ foundB)) dist += prev.distance(pt);
      if (pt.equals(a)) foundA = true;
      if (pt.equals(b)) foundB = true;
  ...
  /**
   * The provided epicenter point may or may not be on the
   * sequence. Wherever it is, there is a point on the sequence that
   * is closest to it. This method runs through all points in the
   * sequence and writes the distance from that closest point to each
   * point, and puts it in the attribute slot with the provided name.
   */
...
//   }
  ...
  public static void getSplinePatch(double x0, double x1, double x2, double x3,
				    double y0, double y1, double y2, double y3,
				    Sequence seq, double numSteps) {
    ...
    boolean debug = false; // (x2 == x3 && y2 == y3);
  ...
  public static int getSplinePatch(Pt a, Pt b, Pt c, Pt d, Sequence seq, int numSteps) {
    int before = seq.size();
    getSplinePatch(a.getX(), b.getX(), c.getX(), d.getX(),
		   a.getY(), b.getY(), c.getY(), d.getY(), seq, numSteps);
    int after = seq.size();
    return after - before;
  }

  /**
   * Gets a spline with some number of control points. To get a
   * straight line, use numCtrlPoints of two.
   */
  ...
  /**
   * From a given index that is a known distance, find the point in
   * some direction (+1 for increasing indexes, -1 for decreasing)
   * that is the specified distance along the sequence. The return
   * value is an interpolated Point based on the surrounding two
   * points that are actually found on the sequence.
   *
   * For example: given the sequence:
   *
   *  0  1  2  3  4  5  6  7    distance between points=10.0
   *  *  o  *  *  *  p  *  *
   *                 +---^
   *  With knownIndex = 5, knownDistance = 40.0, direction = 1, and
   *  desiredDistance = 53, the returned Point is an interpolated
   *  point between points 6 and 7 (closer to 6).
   *
   * @param seq the Sequence to search
   * @param knownIndex the index of a known point p
   * @param knownDistance the distance that known point p is from some
   * other interesting point o
   * @param direction the direction to search through the
   * sequence--this parameter must be 1 for increasing indexes or -1
   * for decreasing indexes.
   * @param desiredDistance the target point t should be from the
   * other interesting point o
   *
   * @return an interpolated point (or, extrapolated if the given
   * distance is outside the first or last point)
   */
  public static Pt getPointAtDistance(Sequence seq, int knownIndex, double knownDistance, 
						int direction, double desiredDistance) {
    if (desiredDistance < knownDistance) throw new RuntimeException("desiredDistance must be greater than or equal to known distance: " + Debug.num(desiredDistance) + " !< " + Debug.num(knownDistance));
    if (direction != 1 && direction != -1) throw new RuntimeException("Direction must be -1 or 1. Provided: " + direction);
  ...
  /**
   * Returns an array of values to be used in a matrix equation for
   * performing an affine transformation. The returned values, in
   * order, are: scaleX, shearY, shearX, scaleY, translateX,
   * translateY. The returned array can be used as the sole argument
   * to initialize a java.awt.geom.AffineTransform. The shear values
   * are always zero and are only returned so you can directly use
   * this method in making an AffineTransform.
   */
  public static double[] getAffineTransform(Rectangle2D source, Rectangle2D dest) {
    ...
    return new double[] { scaleX, 0.0, 0.0, scaleY, transX, transY };
  }

  /**
   * Creates a rectangle that includes the two points, as well as
   * padW/2 and padH/2 surrounding it.
   */
  ...
  public static Rectangle2D growRectangle(Rectangle2D r, double dx, double dy) {
    return new Rectangle2D.Double(r.getX() - (dx/2.0),
				  r.getY() - (dy/2.0),
				  r.getWidth() + dx, 
				  r.getHeight() + dy);
  }
  ...
  /**
   * It is common to need to get the fraction of some value within a
   * range. Assuming your input value is a and the range's lower and
   * upper bounds are b and c respectively, you can use this funtion
   * to get the fraction of a in that range.
   *
   * @param a the value that is to be tested inside some range.
   * @param b the lower (b less than c) value of the range
   * @param c the upper (c greater than b) value of the range
   * 
   * @return (a-b) / (c-b)
   */
  ...
  /**
   * Returns the same as getFraction except the return value is
   * guaranteed to be clamped to the range 0..1.
   */
  ...
  /**
   * Tells you if a ray projected from the given point to the right 
   * (increasing x) intersects with the given line. This uses some simple
   * inequality checks in order to avoid doing math when possible, so it
   * does not calculate the exact intersection point. If you need the exact
